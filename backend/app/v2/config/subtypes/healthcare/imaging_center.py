from app.v2.config.master_config import (
    BuildingType,
    BuildingConfig,
    TradeBreakdown,
    SoftCosts,
    FinancingTerms,
    OwnershipType,
    NLPConfig,
)


CONFIG = (
    BuildingType.HEALTHCARE,
    "imaging_center",
    BuildingConfig(
        display_name="Diagnostic Imaging Center",
        base_cost_per_sf=420,
        cost_range=(420, 560),
        facility_metrics_profile="healthcare_outpatient",
        scope_items_profile="healthcare_imaging_center_structural_v1",
        dealshield_tile_profile="healthcare_imaging_center_v1",
        equipment_cost_per_sf=120,
        typical_floors=1,
        ti_allowance_per_sf=0,
        trades=TradeBreakdown(
            structural=0.24,
            mechanical=0.32,
            electrical=0.24,
            plumbing=0.08,
            finishes=0.12,
        ),
        soft_costs=SoftCosts(
            design_fees=0.09,
            permits=0.02,
            legal=0.02,
            financing=0.03,
            contingency=0.07,
            testing=0.02,
            construction_management=0.04,
            startup=0.02,
        ),
        soft_costs_pct_of_hard=0.25,
        contingency_pct_of_hard=0.07,
        ownership_types={
            OwnershipType.FOR_PROFIT: FinancingTerms(
                debt_ratio=0.65,
                debt_rate=0.065,
                equity_ratio=0.35,
                target_dscr=1.4,
                target_roi=0.10,
            )
        },
        nlp=NLPConfig(
            keywords=[
                "imaging center",
                "diagnostic center",
                "MRI",
                "CT scan",
                "radiology",
                "x-ray",
                "medical imaging",
                "diagnostic imaging",
            ],
            priority=3,
            incompatible_classes=[],
        ),
        regional_multipliers={
            "Nashville": 1.03,
            "Franklin": 1.03,
            "Franklin": 1.03,
            "Manchester": 0.96,
            "Memphis": 0.94,
            "New York": 1.30,
            "San Francisco": 1.35,
            "Chicago": 1.15,
            "Miami": 1.08,
        },
        special_features={
            "mri_suite": {
                "basis": "COUNT_BASED",
                "value": 850000,
                "count": 1,
                "count_pricing_mode": "overage_above_default",
                "count_override_keys": ["mri_suite_count", "mri_count"],
                "unit_label": "suite",
            },
            "ct_suite": {
                "basis": "COUNT_BASED",
                "value": 650000,
                "count": 1,
                "count_pricing_mode": "overage_above_default",
                "count_override_keys": ["ct_suite_count", "ct_count"],
                "unit_label": "suite",
            },
            "pet_scan": {
                "basis": "COUNT_BASED",
                "value": 1100000,
                "count": 1,
                "count_override_keys": ["pet_scan_count", "pet_count"],
                "unit_label": "suite",
            },
            "mammography": {
                "basis": "COUNT_BASED",
                "value": 375000,
                "count": 1,
                "count_pricing_mode": "overage_above_default",
                "count_override_keys": ["mammography_suite_count", "mammography_count"],
                "unit_label": "suite",
            },
            "ultrasound": {
                "basis": "COUNT_BASED",
                "value": 160000,
                "count": 1,
                "count_pricing_mode": "overage_above_default",
                "count_override_keys": ["ultrasound_room_count", "ultrasound_count"],
                "unit_label": "room",
            },
            "hc_imaging_second_mri": {
                "basis": "COUNT_BASED",
                "value": 900000,
                "count": 1,
                "count_override_keys": ["second_mri_count", "additional_mri_count"],
                "unit_label": "suite",
            },
            "hc_imaging_pet_ct_suite": {
                "basis": "COUNT_BASED",
                "value": 1600000,
                "count": 1,
                "count_override_keys": ["pet_ct_suite_count"],
                "unit_label": "suite",
            },
            "hc_imaging_interventional_rad": {
                "basis": "COUNT_BASED",
                "value": 1400000,
                "count": 1,
                "count_override_keys": ["interventional_rad_count", "interventional_suite_count"],
                "unit_label": "suite",
            },
        },
        special_feature_pricing_statuses={
            "mri_suite": "included_in_baseline",
            "ct_suite": "included_in_baseline",
            "pet_scan": "incremental",
            "mammography": "included_in_baseline",
            "ultrasound": "included_in_baseline",
            "hc_imaging_second_mri": "incremental",
            "hc_imaging_pet_ct_suite": "incremental",
            "hc_imaging_interventional_rad": "incremental",
        },
        base_revenue_per_sf_annual=525,
        operating_margin_base=0.22,
        occupancy_rate_base=0.80,
        occupancy_rate_premium=0.88,
        units_per_sf=1.0 / 800.0,
        financial_metrics={
            "primary_unit": "scan rooms",
            "units_per_sf": 1.0 / 800.0,
            "revenue_per_unit_annual": 850000,
            "operational_metrics": {
                "throughput_per_unit_day": 11.0,
                "operating_days_per_year": 310,
                "utilization_target": 0.76,
                "provider_fte_per_unit": 0.22,
                "support_fte_per_provider": 1.7,
                "specialist_fte_per_unit": 0.95,
                "specialist_label": "Imaging Tech FTE",
                "throughput_label": "Scans / Day",
                "utilization_label": "Scanner Utilization",
                "efficiency_label": "Scan Throughput Efficiency",
                "efficiency_green_threshold": 84.0,
                "efficiency_yellow_threshold": 68.0,
            },
        },
        yield_on_cost_hurdle=0.10,
        dscr_target=1.4,
        basis_risk_tolerance_pct=0.10,
        labor_cost_ratio=0.35,
        supply_cost_ratio=0.08,
        management_fee_ratio=0.10,
        insurance_cost_ratio=0.02,
        utility_cost_ratio=0.04,
        maintenance_cost_ratio=0.15,
        equipment_lease_ratio=0.10,
    ),
)

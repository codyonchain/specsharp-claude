...
export const DealShieldView: React.FC<Props> = ({
  ...
  const columns = Array.isArray((viewModel as any)?.columns) ? (viewModel as any).columns : [];
  const rows = Array.isArray((viewModel as any)?.rows) ? (viewModel as any).rows : [];
  ...
  const scenarioInputs = Array.isArray(provenance?.scenario_inputs)
    ? provenance.scenario_inputs
    : Array.isArray((viewModel as any)?.scenario_inputs)
      ? (viewModel as any).scenario_inputs
      : [];
      ...
      {!dealShieldLoading && !dealShieldError && dealShieldData && (
        <>
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Scenario Table</h3>
                      ...
                      <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                        ...
                        {columns.map((_: any, colIndex: number) => {
                          ...
                          const rawValue =
                            ...
                            cell?.label ??
                            cell;
                          const coverage = cell?.coverage ?? cell?.coverage_status;
                          return (
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {scenarioInputs.length > 0 ? (
              ...
              <p className="text-sm text-slate-500">No scenario inputs provided.</p>
            )}

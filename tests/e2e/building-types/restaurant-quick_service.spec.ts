...
test.describe('restaurant - quick_service Tests', () => {
  ...
  test('QSR Nashville - Cost Calculation', async ({ page }) => {
    ...
    await descriptionInput.fill('4200 sf QSR with drive through in Nashville TN');
    ...
    if (costTexts.length > 0) {
      ...
      expect(costPerSqft).toBeGreaterThanOrEqual(350);
      expect(costPerSqft).toBeLessThanOrEqual(400);
    }
    ...
    if (await totalCostElement.isVisible()) {
      ...
      expect(totalCost).toBeGreaterThanOrEqual(1470000);
      expect(totalCost).toBeLessThanOrEqual(1680000);
    }
  });
